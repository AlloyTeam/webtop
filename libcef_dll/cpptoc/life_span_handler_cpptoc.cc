...
int CEF_CALLBACK life_span_handler_on_before_popup(
  ...
  if (!settings)
    return 0;
  // Verify param: newBrowser; type: refptr_diff_byref
  DCHECK(newBrowser);
  if (!newBrowser)
    return 0;
  ...
  // Translate param: newBrowser; type: refptr_diff_byref
  CefRefPtr<CefBrowser> newBrowserPtr;
  if (newBrowser && *newBrowser)
    newBrowserPtr = CefBrowserCToCpp::Wrap(*newBrowser);
  CefBrowser* newBrowserOrig = newBrowserPtr.get();
  ...
  // Restore param: newBrowser; type: refptr_diff_byref
  if (newBrowser) {
    if (newBrowserPtr.get()) {
      if (newBrowserPtr.get() != newBrowserOrig) {
        *newBrowser = CefBrowserCToCpp::Unwrap(newBrowserPtr);
      }
    } else {
      *newBrowser = NULL;
    }
