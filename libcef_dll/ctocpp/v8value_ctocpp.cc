...
bool CefV8ValueCToCpp::ExecuteFunctionWithStringParam(
    CefRefPtr<CefV8Context> context, CefRefPtr<CefV8Value> object,
    const CefString& strargs) {
  if (CEF_MEMBER_MISSING(struct_, execute_function_with_string_param))
    return false;
  ...
  DCHECK(!strargs.empty());
  if (strargs.empty())
    return false;
  // Unverified params: object

  // Execute
  int _retval = struct_->execute_function_with_string_param(struct_,
      CefV8ContextCToCpp::Unwrap(context),
      CefV8ValueCToCpp::Unwrap(object),
      strargs.GetStruct());
