...
bool CefLifeSpanHandlerCToCpp::OnBeforePopup(
  ...
  cef_browser_t* newBrowserStruct = NULL;
  if (newBrowser.get())
    newBrowserStruct = CefBrowserCppToC::Wrap(newBrowser);
  cef_browser_t* newBrowserOrig = newBrowserStruct;
  ...
  if (newBrowserStruct) {
    if (newBrowserStruct != newBrowserOrig) {
      newBrowser = CefBrowserCppToC::Unwrap(newBrowserStruct);
    }
  } else {
    newBrowser = NULL;
  }
